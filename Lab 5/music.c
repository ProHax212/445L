...
Note nullNotes[maxNotes] = {{0,0}, {0,0}};//Default channel if using less than maxChannels
Note singleNote[maxNotes] = {{QUARTER_NOTE,1000}, {0,0}};//Extra null notes if using less than maxNotes
Note doubleNote[maxNotes] = {{HN, 1200}, {HN, 1200}};

//32 outputs for a sine wave
Instrument sine = {.wavePointer=0, .wave = {
...
Instrument flute = {.wavePointer=0, .wave = {
...
void Music_Rewind(){
	...
	for(int c=0; c<sizeof(song->channels)/sizeof(song->channels[0]); c++){//For all channels
...
int Music_Update_Amplitude(){
	...
	for(int c=0; c<sizeof(song->channels)/sizeof(song->channels[0]); c++){//For each channel
		Channel *channel = song->channels[c];
		
		if(channel->notePointer < (sizeof(channel->notes)/sizeof(channel->notes[0]))){//If channel is not finished
	...
	return (amp/(sizeof(song->channels)/sizeof(song->channels[0])));//Average to prevent too high of amplitude
}
...
void Init_Player(){
	...
	Channel singleFluteNote = {.instrument=&flute, .notes={{QUARTER_NOTE, 1000}, {QUARTER_NOTE, 800}}, .notePointer=0, .beatCounter=0, .waitTime=0};
	Song song = {{&singleFluteNote}};
	...
	//player.songs[0]->channels[0] = &singleFluteNote;//Define channels of each song
